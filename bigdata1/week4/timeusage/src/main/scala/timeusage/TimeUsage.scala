...
//    timeUsageGroupedTyped(timeUsageSummaryTyped(summaryDf))
//      .show()
//
//    timeUsageGroupedSql(summaryDf)
//      .show()
  }
  ...
  type Triplet = (List[Column], List[Column], List[Column])

  private val primaryNeeds = List("t01", "t03", "t11", "t1801", "t1803")
  private val workActivities = List("t05", "t1805")
  private val other = List("t02", "t04", "t06", "t07", "t08", "t09", "t10", "t12", "t13", "t14", "t15", "t16", "t18")

  def classifiedColumns(columnNames: List[String]): Triplet = {
    import org.apache.spark.sql.functions._

    def classify(columnNames: List[String], t: Triplet): Triplet = columnNames match {
      case x :: xs if primaryNeeds.exists(x.startsWith) => classify(xs, (t._1 ++ List(column(x)), t._2, t._3))
      case x :: xs if workActivities.exists(x.startsWith) => classify(xs, (t._1, t._2 ++ List(column(x)), t._3))
      case x :: xs if other.exists(x.startsWith) => classify(xs, (t._1, t._2, t._3 ++ List(column(x))))
      case _ :: xs => classify(xs, t)
      case Nil => t
    }
  ...
  def timeUsageGroupedTyped(summed: Dataset[TimeUsageRow]): Dataset[TimeUsageRow] = {
    ...
    def _round: Double => Double = x => (x * 10).round / 10d
    ...
    summed
      ...
      .map {
        case ((working, sex, age), primary, work, otherNeeds) => TimeUsageRow(working, sex, age, _round(primary), _round(work), _round(otherNeeds))
      }.orderBy('working, 'sex, 'age)
  }
}

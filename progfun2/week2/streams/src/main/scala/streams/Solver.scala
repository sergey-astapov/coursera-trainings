...
trait Solver extends GameDef {
  ...
  def from(initial: Stream[(Block, List[Move])],
           explored: Set[Block]): Stream[(Block, List[Move])] =
    if (initial.nonEmpty) {
      val strm = initial.flatMap {
        case (b, m) => newNeighborsOnly(neighborsWithHistory(b, m), explored)
      }
      initial #::: strm #::: from(strm, strm.map(x => x._1).toSet ++ explored)
    } else Stream.empty
   ...
   */
  lazy val pathsToGoal: Stream[(Block, List[Move])] = pathsFromStart.filter(x => x._1.isLegal && done(x._1))
